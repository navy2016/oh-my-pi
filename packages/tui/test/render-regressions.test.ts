...
describe("TUI terminal-state regressions", () => {
	...
	describe("scrollback integrity", () => {
		...
		it("preserves bottom-anchored viewport across an offscreen middle-shrink on unknown win32", async () => {
			// Repro for #1809: `win32-unknown-small` blanked the visible window after a
			// `deleteMiddle` far above the viewport. The shrink fell into the deferred
			// fallback, which blank-pads the tail to keep pre-shrink absolute indices —
			// correct only for trailing shrinks. An offscreen middle deletion shifts
			// later rows up, so the padded slice at the old viewport indices showed
			// post-shift content and dropped the live rows. The fix keeps the no-op
			// when the new bottom-anchored window equals the displayed one.
			const originalPlatform = process.platform;
			Object.defineProperty(process, "platform", { configurable: true, value: "win32" });
		...
		it("repaints the viewport when an offscreen shrink also changes the visible tail", async () => {
			const originalPlatform = process.platform;
			Object.defineProperty(process, "platform", { configurable: true, value: "win32" });
			const term = new UnknownViewportTerminal(32, 6);
			const tui = new TUI(term);
			...
			try {
				tui.start();
				await settle(term);
				// Offscreen deletion AND a visible tail change: the no-op would freeze
				// the stale tail, so the renderer must repaint the true bottom.
		...
		it("preserves a cursor-only tail move while deferring an offscreen shrink on unknown win32", async () => {
			const originalPlatform = process.platform;
			Object.defineProperty(process, "platform", { configurable: true, value: "win32" });
			const term = new UnknownViewportTerminal(32, 6);
			const tui = new TUI(term);
			...
			try {
				...
				// Offscreen deletion; the visible rows are byte-identical after stripping
				// CURSOR_MARKER, but the caret moves to the end. The deferred no-op must
				// still emit the remapped cursor.
		...
		it("preserves bottom-anchored viewport across an offscreen middle-shrink on unknown ED3 eager-off", async () => {
			// The same guard covers the ED3-risk eager-off deferredShrink site (the
			// non-forced path, where the viewport probe is unobservable and the user
			// may be following the live tail). An offscreen-only delete must keep the
			// visible rows, not blank-pad shifted content.
